////// Enums //////
export enum ExtensionAnchors {
  component = "component",
  panel = "panel",
  videoOverlay = "video_overlay",
}

export enum HelperModes {
...
export interface ITwitchJwt {
  ...
  is_unlinked: boolean;
  /**
   * Identifies the session using this JWT. Any token generated by your EBS should leave this field blank. When present,
   * the value is interpreted as follows:
   * - Values beginning with "U" are stable references to a Twitch account across sessions and channels. You can use them
   * to provide persistent services to users.
   * - Values beginning with "A" are transient references to an anonymous Twitch session; the user has not logged into the
   * Twitch website. These values are not stable and should never be associated with persistent data. It is possible for them
   * to represent different users over time. If your extension requires a stable user identity, your front-end interface
   * should display an appropriate login request when this field does not begin with "U."
   *
   * Broadcasters’ tokens are set with "U" + their Twitch user IDs, to avoid confusing opaque IDs with user IDs when the
   * broadcaster is a viewer.
   */
  opaque_user_id: string;
  /**
   * Defines the ability of the token holder to send and listen to messages for your extension. `pubsub_perms` contains two arrays:
   * `listen` and `send`, which contain the topics the associated user is allowed to listen to and publish to, respectively.
   *
   ...
   * A list of specific values means only the specified targets are allowed. If a permission is absent, it is equivalent to the empty
   * listing: there are no default allowed targets. When sending messages from the EBS, specify an asterisk as the `send` permission, to
   * allow the message to pass through the system.
   */
  pubsub_perms: object;
  /** Type of user for whom this JWT has been signed. This is required. */
  role: Roles;
  /**
   * The user’s Twitch user ID. This is provided only for users who allow your extension to identify them. There are no guarantees this
   * will be available, as users can revoke an extension's ability to identify them at any time. Users who revoke an extension's ability
   * to identify them are issued a new opaque identifier.
   */
  user_id: string;
}
...
export interface IBitsTransaction {
  transactionId: string;
  product: IBitsProduct;
  userId: string;
  displayName: string;
  initiator: any;
...
export interface IConfiguration {
   ...
   */
  set(segment: ConfigSegments, version: string, content: string): void;
}
...
export interface IBits {
   ...
   */
  getProducts(): Promise<IBitsProduct[]>;
   ...
   */
  onTransactionComplete(callback: (transactionObject: IBitsTransaction) => void): void;
...
export interface TwitchExtensionHelper {
   ...
   */
  onAuthorized(authCallback: (auth: ITwitchAuth) => void): void;

  /**
   * This callback is fired when the context of an extension is fired.
   * @param contextCallback is a function with two arguments, an `ITwitchContext` object and an array of `string`s naming the `ITwitchContext` properties that were changed.
   */
  onContext(contextCallback: (context: ITwitchContext, delta: string[]) => void): void;
   ...
   * @param callback A callback that is called with one or two arguments: a boolean indicating whether the extension is visible, and a fresh `ITwitchContext` object.
   * The second argument is present only when `isVisible` is true.
   */
  onVisibilityChanged(callback: (isVisible: boolean, context?: ITwitchContext) => void): void;
   ...
   */
  send(target: MessageTargets | string, contentType: string, message: object): void;
  send(target: MessageTargets | string, contentType: string, message: string): void;
   ...
   */
  listen(target: MessageTargets | string, callback: (target: MessageTargets | string, contentType: string, message: string) => void): void;
   ...
   */
  unlisten(
    target: MessageTargets | string,
    callback: (target: MessageTargets | string, contentType: string, message: string) => void
  ): void;
...
declare global {
  interface Window {
    Twitch: { ext: TwitchExtensionHelper };
  }
}
